...
def feature_extractor(data):
    ...
    x = data["TL BASED ISE"]
    data.drop(columns=["TL BASED ISE"], inplace=True)
    data["y"] = x
    return data


working_dir = os.getcwd()
data = pd.read_csv(f"{working_dir}/Datasets/Istanbul_Stock_Exchange/data_akbilgic.csv", index_col="date")
data = feature_extractor(data)
data.to_csv(f"{working_dir}/Datasets/Istanbul_Stock_Exchange/data_akbilgic_extracted.csv", index=False)
