...
def feature_extractor(data):
    ...
    x = data["Appliances"]
    data.drop(columns=["Appliances"], inplace=True)
    data["y"] = x
    return data


working_dir = os.getcwd()
data = pd.read_csv(f"{working_dir}/Datasets/Appliances_Energy_Prediction/energydata_complete.csv", index_col="date")
data = feature_extractor(data)
data.to_csv(f"{working_dir}/Datasets/Appliances_Energy_Prediction/energydata_complete_extracted.csv", index=False)
